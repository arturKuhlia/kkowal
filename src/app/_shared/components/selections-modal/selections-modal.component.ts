...
@Component({
  selector: 'app-selections-modal',
  templateUrl: './selections-modal.component.html',
  styleUrls: ['./selections-modal.component.scss']
})
export class SelectionsModalComponent implements OnInit {
  @ViewChild('selectionForm', { static: true }) selectionForm: NgForm;

  heading: string;
  selection: Selection = {};

  selectionData: Subject<Selection> = new Subject();
...
// }
  onSave() {
    if (this.selectionForm.valid) {
      this.selectionData.next(this.selection);
      this.modalRef.hide();
    } else {
